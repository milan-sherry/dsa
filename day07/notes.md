...
use result(n,1) to have n with initailized 1 ifonly result(n) then also fine but if not written like this we cant access indexs
...
238. For this you have to use suffic and prefix so find the suffix of each index then next loop multipy with the prefix of every index
